...
async function fetchHtml(url) {
  ...
  try {
    const res = await axios.get(requestUrl, { timeout: 20000, headers });
    return res.data;
  } catch (error) {
    if (error.response?.status !== 403 || !url.includes("pastec.net")) throw error;
    const retry = await axios.get(requestUrl, {
      timeout: 20000,
